...
class WalkForwardSplit:
    ...
    def __init__(self, n_splits: int = 5, embargo_bars: int = 3, min_train_size: int = 10000):
...
def main():
    ...
    if len(X_sequences) == 0:
        logger.error("No sequences generated - insufficient data")
        return
    
    logger.info(f"Generated {len(X_sequences)} sequences with shape {X_sequences.shape}")
    logger.info(f"Class distribution: {pd.Series(y_sequences).value_counts().sort_index().to_dict()}")
